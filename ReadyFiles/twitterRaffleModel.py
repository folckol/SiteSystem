...
class Account:
    ...
    def __init__(self, accs_data):
        ...
        self.csrf = accs_data['csrf']
        self.auth_token = accs_data['auth_token']
    ...
    def execute_tasks(self, RaffleInfo):
        ...
        tweetId = RaffleInfo['Link'].split('/')[-1].replace('?s=20', '')
        self.status = 'Success'
        list_ = []

        if RaffleInfo['LikeStatus']:
            list_.append(self.Like(tweetId))
            time.sleep(random.randint(100,350)/100)

        if RaffleInfo['RetweetStatus']:
            list_.append(self.Retweet(tweetId))
            time.sleep(random.randint(100, 350) / 100)
        ...
        if RaffleInfo['CommentStatus']:
            ...
            try:
                ...
                if CommentData['MaxTags'] != 0 and (CommentData['MaxTags'] != 1 and CommentData['MinTags'] != 1):
                    ...
                    if minTags == maxTags:
                        ...
                        tagsCount = random.randint(minTags, maxTags+1)
                ...
                list_.append(self.Reply(tweetId, tweetText))
    ...
    def Like(self, id):
        ...
        with self.session.post("https://twitter.com/i/api/graphql/lI07N6Otwv1PhnEgXILM7A/FavoriteTweet",  json=payload, timeout=5) as response:
            if response.ok:
    ...
    def Retweet(self, id):
        ...
        with self.session.post("https://twitter.com/i/api/graphql/ojPdsZsimiJrUGLR1sjUtA/CreateRetweet", json=payload, timeout=30) as response:
            if response.ok:
                return response.json()
    ...
    def Follow(self, user_id):
        ...
        with self.session.post(f"https://twitter.com/i/api/1.1/friendships/create.json?user_id={user_id}&follow=True", timeout=30) as response:
            # print(response.text)
    ...
    def Tweet(self, text, mediaId):
        ...
        if mediaId == None:
            ...
            with self.session.post("https://twitter.com/i/api/graphql/Tz_cZL9zkkY2806vRiQP0Q/CreateTweet", json=payload,
            ...
            self.session.headers.update({'content-type': 'application/json'})
            with self.session.post("https://twitter.com/i/api/graphql/7TKRKCPuAGsmYde0CudbVg/CreateTweet", json=payload,
                                   timeout=30) as response:
    ...
    def Get_Tweets(self):
        ...
        recomend_tweets = []
        with self.session.post('https://twitter.com/i/api/graphql/D8Tklm7zoICDtod5RCC6qg/HomeTimeline', json=payload, timeout=5) as response:
    ...
    def Reply(self, tweet_id, text):
        ...
        with self.session.post('https://twitter.com/i/api/graphql/Tz_cZL9zkkY2806vRiQP0Q/CreateTweet', json=payload, timeout=10) as response:
            return response.json()
    ...
    def Update_profile_image(self, media_id):
        ...
        with self.session.post('https://twitter.com/i/api/1.1/account/update_profile_image.json', data=payload,
                               timeout=20) as response:
            print(response.text)
    ...
    def Update_profile_banner(self, media_id):
        ...
        with self.session.post('https://twitter.com/i/api/1.1/account/update_profile_banner.json', data=payload,
                               timeout=20) as response:
            print(response.text)
    ...
    def Update_profile_data(self, name, description, location):
        ...
        with self.session.post('https://twitter.com/i/api/1.1/account/update_profile.json', data=payload,
                               timeout=20) as response:
            print(response.text)
    ...
    def Change_username(self):
        ...
        with self.session.post('https://twitter.com/i/api/1.1/account/settings.json', data=payload,
                               timeout=10) as response:
    ...
    def Get_User_Id(self, name):
        ...
        with self.session.get(f'https://twitter.com/i/api/1.1/users/show.json?screen_name={name}', timeout=10, allow_redirects=True) as response:

            return response.json()['id_str']

    def Get_User_Followers(self, name):

        with self.session.get(f'https://twitter.com/i/api/1.1/users/show.json?user_id={name}', timeout=10, allow_redirects=True) as response:
            # print(response.text)
    ...
    def add_visions(self, id):
        ...
        with self.session.post(f'https://twitter.com/i/api/1.1/live_pipeline/update_subscriptions',data = payload, timeout=10) as response:
            time.sleep(1)
            pass
    ...
    def SearchTweets(self):
        ...
        with self.session.get('https://twitter.com/i/api/1.1/search/tweets.json', params=payload, timeout=10) as response:
            # print(response.text)
            return response.json()
